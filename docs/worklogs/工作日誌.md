...
- **17:00-18:00** － 專案結構整理與 GitHub 同步
  - 建立 `config/private/`、`docs/private/` 管理敏感憑證與營運資料，更新 `.gitignore` 規則。
  - 移除子 repo (`demos/ecommerce-growth-planner/.git`)，清除 `*.DS_Store` 與自動產生的 `logs/`、`output/`。
  - `git add` 公開範圍（demos、docs、specs、tools、backend/frontend/data 等）並 `git commit -m "Organize repo structure and add tracked assets" && git push origin main`。
  - 確認 `git status -sb` 為乾淨狀態，後續跨環境可依 `config/README.md` 進行還原。
